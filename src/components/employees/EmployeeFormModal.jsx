...
export default function EmployeeFormModal({ open, onClose, onSave, posList = [], roleList = [], employee }) {
  ...
  async function handleSubmit(e) {
    e.preventDefault();
    setError("");
    if (!form.nombre || !form.apellido || !form.email || !form.dni) return setError("Nombre, apellido, email y DNI son obligatorios.");
    setSaving(true);
    try {
      await onSave({ ...form, permisos });
      onClose();
    } catch (err) {
    ...
    <Modal open={open} onClose={onClose} title={isEdit ? "Editar empleado" : "Nuevo empleado"} width="max-w-2xl">
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div><label className="label">Nombre</label><input className="input" value={form.nombre} onChange={(e) => set("nombre", e.target.value)} /></div>
          <div><label className="label">Apellido</label><input className="input" value={form.apellido} onChange={(e) => set("apellido", e.target.value)} /></div>
          ...
          <div><label className="label">Contraseña {isEdit && "(dejar vacío = sin cambios)"}</label><input className="input" type="password" value={form.password} onChange={(e) => set("password", e.target.value)} /></div>
          <div>

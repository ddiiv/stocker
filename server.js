/*
 * Servidor de producción del BACKOFFICE.
 *
 * Hace dos cosas:
 ...
 */
const PORT = Number(process.env.PORT || process.env.BACKOFFICE_PORT) || 8081;
...
const server = app.listen(PORT, '::', () => {
  ...
  if (!dir) return;
  const origenPuerto = process.env.PORT ? 'PORT' : process.env.BACKOFFICE_PORT ? 'BACKOFFICE_PORT' : 'valor por defecto';
  ...
  console.log('─────────────────────────────────────────');
  console.log(`  Backoffice escuchando en ${dir.address}:${dir.port}  (puerto de ${origenPuerto})`);
